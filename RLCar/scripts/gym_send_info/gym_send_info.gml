//Send the list of information(Values you want to use but don't want in the observation)
function gym_send_info(float_list) {
    var buffer = -1;
    try {
        ...
        if (bytes_sent < 0) {
            ...
            show_debug_message("Sent information list and its size successfully");
        }
        
        */
    } catch (exception) {
        show_debug_message("An error occurred in gym_send_info: " + exception);
    } finally {
        if (buffer != -1) {
